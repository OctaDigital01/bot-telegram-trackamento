...
async def job_etapa3_galeria(context: ContextTypes.DEFAULT_TYPE, chat_id_manual=None):
    ...
    # Delay de 4 segundos antes de enviar o texto longo
    await asyncio.sleep(4)
    
    text_vip = """Gostou do que viu? 😏

Essa <b>é só uma PRÉVIA</b> borrada do que te espera bb... 

Tenho muito mais no VIP pra você <b>(TOTALMENTE SEM CENSURA)</b>:
...
Vem goz.ar po.rra quentinha pra mim🥵💦⬇️"""
    ...
    msg = await context.bot.send_message(chat_id=chat_id, text=text_vip, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
...
async def job_etapa3_remarketing(context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("VER SURPRESINHA 🔥", callback_data='trigger_etapa4')]]
...
async def job_etapa4_planos_vip(context: ContextTypes.DEFAULT_TYPE, chat_id_manual=None):
    ...
    texto_planos = (
        ...
        "💎 Chamada de vídeo só nós 2\n"
        "💎 E muito mais meu bem...\n\n"
        "<b>Escolhe qual você quer meu bem:</b>"
    )
